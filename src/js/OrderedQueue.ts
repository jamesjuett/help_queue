    ...
    constructor(elem: JQuery, appointments: HasAppointments, mapImgSrc?: string) {
        ...
        this.signUpForm = $('<form id="signUpForm" role="form" class="form-horizontal"></form>')
            .append(regularFormElem = $('<div></div>')
                ...
                .append($('<div class="form-group"></div>')
                    .append('<label class="control-label col-sm-3" for="signUpLocation' + this._inst_id + '">Meeting Link:</label>')
                    .append($('<div class="col-sm-9"></div>')
                        .append(this.signUpLocationInput = $('<input type="text" class="form-control" id="signUpLocation' + this._inst_id + '"required="required" maxlength="500" placeholder="">'))
                    )
                )
